...
def load_inputs(gems_csv: str, eluc_csv: str, y0: int, y1: int) -> pd.DataFrame:
    years = np.arange(y0, y1+1)
    df = pd.DataFrame({"year": years})

    # FF (GEMS)
    if os.path.exists(gems_csv):
        ff = pd.read_csv(gems_csv)
    ...
    df["FF_GtC"] = df["FF_GtC"].interpolate().fillna(0.0)
    df["ELUC_GtC"] = df["ELUC_GtC"].interpolate().fillna(0.0)
    df["FF_GtC"] = df["FF_GtC"].clip(lower=0)
    df["ELUC_GtC"] = df["ELUC_GtC"].clip(lower=0)
    return df
